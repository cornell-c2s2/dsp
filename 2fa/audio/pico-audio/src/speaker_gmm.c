...
int64_t mfcc_target_speaker_llr_mean(float *mfcc_feats, int num_frames) {
  int16_t feature_vector[GMM_DIMENSION];
  int64_t llr_sum = 0;
  for (int i = 0; i < num_frames; i++) {
    float_to_g6int16_arr(&mfcc_feats[i * GMM_DIMENSION], feature_vector,
                         GMM_DIMENSION);
    llr_sum += target_speaker_llr(feature_vector);
  }
  return llr_sum / num_frames;
}

int classify_speaker(float *mfcc_feats, int num_frames) {
  int64_t mean_llr = mfcc_target_speaker_llr_mean(mfcc_feats, num_frames);
  return (mean_llr > THRESHOLD_INT) ? 1 : 0;
}
