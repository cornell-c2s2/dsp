...
class AudioClassifier:
    ...
    def load_data(self, pos_dir, neg_dir):
        ...
        pos_files = [f for f in all_pos_files if f.endswith(('.wav', '.mp3', '.flac', '.WAV', '.MP3', '.FLAC', '.opus'))]
        ...
        neg_files = [f for f in all_neg_files if f.endswith(('.wav', '.mp3', '.flac', '.WAV', '.MP3', '.FLAC', '.opus'))]
        ...
        if pos_count == 0 and neg_count == 0:
            raise ValueError("No audio files loaded! Please check your data directories and file formats.")
        if pos_count == 0:
            raise ValueError("No positive samples loaded! Please add audio files to the positive directory.")
        if neg_count == 0:
            raise ValueError("No negative samples loaded! Please add audio files to the negative directory.")
    ...
    def predict_directory(self, test_dir):
        ...
        test_files = [f for f in all_files if f.endswith(('.wav', '.mp3', '.flac', '.WAV', '.MP3', '.FLAC', '.opus'))]
        ...
        for filename in test_files:
            ...
            if label is not None:
                class_name = "POSITIVE (contains 'stop')" if label == 1 else "NEGATIVE (no 'stop')"
    ...
    def evaluate_directory(self, test_dir, save_report=True):
        ...
        print(f"  Accuracy: {correct_positive/total_positive:.4f}" if total_positive > 0 else "  Accuracy: N/A")
        
        print(f"\nNegative class (no 'stop'):")
        print(f"  Total: {total_negative}")
        print(f"  Correct: {correct_negative}")
        print(f"  Accuracy: {correct_negative/total_negative:.4f}" if total_negative > 0 else "  Accuracy: N/A")
        ...
        if len(misclassified) > 0:
            ...
            for r in misclassified:
                ...
                print(f"  {r['filename']}")
                print(f"    True: {true_class}, Predicted: {pred_class}, Confidence: {r['confidence']:.4f}")
        else:
        ...
        if save_report:
            self._save_evaluation_report(results, accuracy, precision, recall, f1, conf_matrix, test_dir)
    ...
    def _plot_evaluation_results(self, conf_matrix, y_scores, y_true, test_dir):
        ...
        if len(positive_scores) > 0:
            axes[1].hist(positive_scores, bins=20, alpha=0.6, label='Positive (with stop)', 
                        color='green', edgecolor='black')
        if len(negative_scores) > 0:
            axes[1].hist(negative_scores, bins=20, alpha=0.6, label='Negative (no stop)', 
    ...
    def _save_evaluation_report(self, results, accuracy, precision, recall, f1, conf_matrix, test_dir):
        """
        ...
        with open(report_path, 'w') as f:
            ...
            sorted_results = sorted(results, key=lambda x: (x['correct'], -x['confidence']))
            ...
            for r in sorted_results:
                ...
                f.write(f"   True: {true_class:8s} | Predicted: {pred_class:8s} | Confidence: {r['confidence']:.4f}\n\n")
...
def main():
    ...
    classifier = AudioClassifier(n_mfcc=13, max_length=100)
