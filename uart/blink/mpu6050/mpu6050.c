...
void mpu6050_read_raw(int16_t accel[3], int16_t gyro[3]) {
     ...
     uint8_t val = 0x3B;
     i2c_write_blocking(i2c_default, ADDRESS, &val, 1, true); // true to keep master control of bus
     ...
     i2c_read_blocking(i2c_default, ADDRESS, buffer, 6, false);  // False - finished with bus
